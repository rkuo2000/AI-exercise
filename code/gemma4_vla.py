...
# pip install onnx_asr kokoro_onnx
...
# ── Config (override with env vars before launching) ──(hw:card,device)──────────
OLLAMA_URL = os.getenv("OLLAMA_URL", "http://127.0.0.1:11434/v1/chat/completions")
MIC       = os.getenv("MIC_DEVICE", "hw:3,0")
#SPK       = os.getenv("SPK_DEVICE", "hw:0,1") # to Analog ALC897
SPK       = os.getenv("SPK_DEVICE", "hw:3,1") # to onboard HDMI 
...
def llm(messages, tools=None):
    body = {"model": "gemma4-turbo-128k:e2b", "messages": messages, "max_tokens": 500,
...
{BD}{CY}   ██████  ███████ ███    ███ ███    ███  █████      ██   ██
  ...
  ██    ██ ██      ██  ██  ██ ██  ██  ██ ██   ██          ██
   ██████  ███████ ██      ██ ██      ██ ██   ██          ██{R}
""")
    print(f"  {YL}{'RTX5060-16GB · 100% Local · gemma4-turbo:e2b ':^{W}}{R}")
    print(f"  {BL}{'▀' * W}{R}\n")
    ...
    if not text_mode:
        ...
        ln = "─" * W
        print(f"  {DM}{ln}{R}")
        print(f"  {DM}STT: Parakeet 0.6B · TTS: Kokoro 82M · LLM: Gemma4-Turbo:E2B · {time.time()-t0:.0f}s{R}")
        print(f"  {DM}{ln}{R}\n")
        play_prompt("hello")
